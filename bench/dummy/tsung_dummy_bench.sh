...
WORKING_DIR=./bench/dummy
TSUNG_CONFIG_FILE=$WORKING_DIR/dummy.xml
...
./scripts/tsung_test.sh $TSUNG_CONFIG_FILE  $WORKING_DIR/normal    "python3 ethermint-dos.py 4 0 --dummy --verbosity 1 --no-docker"
for pid in $(ps -ef | grep "tendermint" | awk '{print $2}'); do sudo kill -9 $pid; done

echo "***********************************************"
echo "Test evil network (with one byzantine node)"
./scripts/tsung_test.sh $TSUNG_CONFIG_FILE  $WORKING_DIR/evil      "python3 ethermint-dos.py 4 1 --dummy --verbosity 1 --no-docker"
for pid in $(ps -ef | grep "tendermint" | awk '{print $2}'); do sudo kill -9 $pid; done
